...
def main():
    canvas = LogicCanvas()
    id_1_sigma = canvas.Ent("sigma", in_list=['B', 'A'], out_list=['A','B'])
    id_2_gamma = canvas.Ent("gamma", in_list=['A', 'B'], out_list=['A^B'])
    id_3_f     = canvas.Ent("f",     in_list=['A^B'], out_list=['C'])

    ret, id_4_comp = canvas.Comp(id_2_gamma, id_3_f)
    ret, id_5_comp = canvas.Comp(id_1_sigma, id_4_comp)

    ret, id_6_lamb = canvas.Lamb(id_5_comp, ("#0", 'B'), ("#2", 'C'))
    ret, id_7_up = canvas.Up(id_6_lamb, ("#0", 'T'))
    ret, id_8_lamb = canvas.Lamb(id_7_up, ("#0", 'A'), ("#5", 'B=>C'))
    ...
    graph_path = os.path.join(outputs_dir, "graph_A13R1.dot")

    canvas.get_block(id_8_lamb).dump()
    # canvas.get_block(id_8_lamb).create_graph(graph_path, format='png')
    # canvas.get_block(id_8_lamb).create_graph(graph_path, format='svg')
    canvas.create_graph(id_8_lamb, graph_path, format="png")
    canvas.create_graph(id_8_lamb, graph_path, format="svg")
